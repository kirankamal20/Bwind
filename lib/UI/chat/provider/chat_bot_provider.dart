...
class ChatBotListNotifier extends StateNotifier<List<ChatBot>> {
  ...
  Future<List<String>> getChunksFromPDF(String filePath) async {
    ...
    await Isolate.spawn(isolateTask, [filePath, receivePort.sendPort]);

    final List<String> pageTextChunks = await receivePort.first;

    return pageTextChunks;
  }
}

void isolateTask(List<dynamic> message) async {
  final String filePath = message[0];
